...
def EMNIST_HCapsNet(input_shape,
                    no_coarse_class,
                    no_fine_class,
                    PCap_n_dims = 8,SCap_n_dims = 16,
                    n_hidden1 = 512,n_hidden2 = 1024):
...
def MarineTree_HCapsNet(input_shape,
                no_coarse_class,no_medium_class,no_fine_class,
                PCap_n_dims = 8,SCap_n_dims = 16,
                n_hidden1 = 512,n_hidden2 = 1024):
    # Input image
    ...
    model = keras.Model(
        inputs= [x_input, coarse1_input, coarse2_input, fine_input],
        outputs= [coarse_pred_layer, medium_pred_layer, fine_pred_layer, Output_Layer],
        name='H-CapsNet_M_TREE')
    
    return model

def MarineTree_HCapsNet_wENCwoFE(input_shape,
                                no_coarse_class,no_medium_class,no_fine_class,
                                PCap_n_dims = 8,SCap_n_dims = 16,
                                n_hidden1 = 512,n_hidden2 = 1024):
    # Input image
    ...
    # Layer 3: Reshape to 8D primary capsules Coarse
    reshapec1 = keras.layers.Reshape((int((tf.reduce_prod(conv2.shape[1:]).numpy())/PCap_n_dims), PCap_n_dims)
                                       , name="reshape_layer_c1")(conv2)
    squashc1 = keras.layers.Lambda(squash, name='squash_layer_c1')(reshapec1)

    # Layer 4: Reshape to 8D primary capsules Medium
    reshapec2 = keras.layers.Reshape((int((tf.reduce_prod(conv2.shape[1:]).numpy())/PCap_n_dims), PCap_n_dims)
                                       , name="reshape_layer_c2")(conv2)
    squashc2 = keras.layers.Lambda(squash, name='squash_layer_c2')(reshapec2)

    # Layer 5: Reshape to 8D primary capsules Fine
    reshapef = keras.layers.Reshape((int((tf.reduce_prod(conv2.shape[1:]).numpy())/PCap_n_dims), PCap_n_dims)
                                       , name="reshape_layer_f")(conv2)
    squashcf = keras.layers.Lambda(squash, name='squash_layer_f')(reshapef)
...
def MarineTree_HCapsNet_woENCwFE(input_shape,
                        no_coarse_class,no_medium_class,no_fine_class,
                        PCap_n_dims = 8,SCap_n_dims = 16,
                        n_hidden1 = 512,n_hidden2 = 1024):
    # Input image
...
def MarineTree_HCapsNet_woENCwoFE(input_shape,
                        no_coarse_class,no_medium_class,no_fine_class,
                        PCap_n_dims = 8,SCap_n_dims = 16,
                        n_hidden1 = 512,n_hidden2 = 1024):
    # Input image
    ...
    # Layer 3: Reshape to 8D primary capsules 
    reshapec1 = keras.layers.Reshape((int((tf.reduce_prod(x_input.shape[1:]).numpy())/PCap_n_dims), PCap_n_dims)
                                       , name="reshape_layer_c1")(x_input)
    squashc1 = keras.layers.Lambda(squash, name='squash_layer_c1')(reshapec1)

    reshapec2 = keras.layers.Reshape((int((tf.reduce_prod(x_input.shape[1:]).numpy())/PCap_n_dims), PCap_n_dims)
                                       , name="reshape_layer_c2")(x_input)
    squashc2 = keras.layers.Lambda(squash, name='squash_layer_c2')(reshapec2)

    reshapef = keras.layers.Reshape((int((tf.reduce_prod(x_input.shape[1:]).numpy())/PCap_n_dims), PCap_n_dims)
                                       , name="reshape_layer_f")(x_input)
    squashcf = keras.layers.Lambda(squash, name='squash_layer_f')(reshapef)
